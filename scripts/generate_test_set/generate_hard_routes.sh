...
ROUNDABOUT_PATH="test_files/roundabout/hard"
INTERSECTION_PATH="test_files/intersection/hard"
...
convert_single_osm() {
    ...
    netconvert --osm-files "$OSM_FILE" --output-file "$MYPATH/outputs/${BASE_NAME}_dict/${BASE_NAME}/${BASE_NAME}.net.xml" --default.junctions.keep-clear false --tls.red.time 20
    ...
    $SUMO_HOME/tools/randomTrips.py -n "$MYPATH/outputs/${BASE_NAME}_dict/${BASE_NAME}/${BASE_NAME}.net.xml" -p 0.72 -o "$MYPATH/outputs/${BASE_NAME}_dict/${BASE_NAME}/${BASE_NAME}.trips.xml" --fringe-factor max --validate

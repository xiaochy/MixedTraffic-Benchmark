...
from core.costomized_data_structures import Namespace  # delete core.
...
BACK_DISTANCE = 20
WIDTH = 10
MAX_FRONT_DIST = math.sqrt(WIDTH**2 + FRONT_DISTANCE**2)
MAX_BACK_DIST = math.sqrt(WIDTH**2 + BACK_DISTANCE**2)
...
class SubscribeDef:
    ...
    def get(self, *id):
        ...
        return False, Namespace(((n, res[v]) for n, v in zip(self.names, self.constants)))
...
class SUMO(object):
    ...
    def get_vehicle_length(self, veh_id):
        return self.tc.vehicle.getLength(veh_id)

    def get_vehicle_route(self, veh_id):
        return self.tc.vehicle.getRoute(veh_id)
    ...
    def get_position(self, veh):
        return np.array(self.tc.vehicle.getPosition(veh.id))

    def get_speed(self, veh):
        return self.tc.vehicle.getSpeed(veh.id)
    
    def get_lane(self, veh):
        return self.tc.vehicle.getLaneID(veh.id)
    
    def get_lane_position(self,veh):
        return self.tc.vehicle.getLanePosition(veh.id)
    
    def get_acceleration(self, veh):
        return self.tc.vehicle.getAcceleration(veh.id)

    def get_velocity_angle_direction(self,veh):
        """
        direction: unit vector
        angle_radian: from x-axis counterclockwise
        """
        speed = self.tc.vehicle.getSpeed(veh.id)
        angle_degree = self.tc.vehicle.getAngle(veh.id)
        angle_radian = 2*math.pi - math.radians(angle_degree)
        direction = np.array([math.cos(angle_radian), math.sin(angle_radian)])
        velocity = speed * direction
        return velocity, angle_radian, direction

    def get_vehicle_passing(self, JuncID):
        return self.tc.junction.getContextSubscriptionResults(JuncID)
    ...
    def normalize(self, list):
        ...
        if max_elem == min_elem:
            max_elem += 1
        return [(x - min_elem) / (max_elem - min_elem) for x in list]
    ...
    def accl_control(self, veh, acc, n_acc_steps=1):
        ...
        self.tc.vehicle.setSpeed(veh.id, max(0, veh.speed + acc * self.sim_step))
    def IDM_accl_control(self, veh, acc, n_acc_steps=1):
        self.tc.vehicle.setSpeed(veh.id, max(1, veh.speed + acc * self.sim_step))
